...
# Run this script to create the database user in your Docker container

echo "Setting up database user 'parking_app'..."

docker exec -i parking-db psql -U postgres << EOF
...
docker exec -i parking-db psql -U postgres -d parking_assistant << EOF
